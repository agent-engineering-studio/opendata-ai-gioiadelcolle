  ...
  return (
    <Section id="sintesi">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 26 }}>
        {a.kpis.map((k) => (
          <div key={k.label} style={{ background: "#fff", border: "1px solid #E6DECF", borderRadius: 5, padding: "18px 18px 16px" }}>
            <div style={{ font: "600 clamp(28px,3.6vw,40px)/1 'Spectral'", color: "#211E1A" }}>{k.value}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16 }}>
        {a.highlights.map((h) => (
          <div key={h.label} style={{ background: "#fff", border: "1px solid #E6DECF", borderLeft: `4px solid ${h.color}`, borderRadius: 5, padding: "22px 24px" }}>
            <div style={{ font: "600 clamp(34px,5vw,52px)/1 'Spectral'", color: h.color }}>{h.value}</div>
